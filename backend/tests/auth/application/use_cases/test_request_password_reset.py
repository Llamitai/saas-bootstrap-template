...
from src.auth.application.use_cases.password_reset.requester import PASSWORD_RESET_TTL, PasswordResetRequester
...
async def test_execute__known_user_issues_token_and_dispatches_email(
    ...
    expect(command.context["reset_url"]).to(equal("https://app.example.com/reset_password/reset.jwt.token"))
    expect(command.context["name"]).to(equal(user.first_name))


async def test_execute__unknown_email_is_silent_noop(user_repository, token_service, command_bus):
...
async def test_execute__normalizes_email_lowercase_and_trimmed(user, user_repository, token_service, command_bus):
    user_repository.find_by_email.return_value = user
    token_service.create_one_shot_token = AsyncMock(
        return_value="t",
    )
...
async def test_execute__strips_trailing_slash_from_reset_base_url(user, user_repository, token_service, command_bus):
    user_repository.find_by_email.return_value = user
    token_service.create_one_shot_token = AsyncMock(
        return_value="t",
    )
    ...
    command = command_bus.dispatch.call_args.kwargs["command"]
    expect(command.context["reset_url"]).to(contain("https://app.example.com/reset_password/"))
