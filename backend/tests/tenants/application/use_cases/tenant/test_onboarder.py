...
:class:`TenantOnboarder` delegates to :class:`TenantMemberInviter`.
"""
...
async def test_execute__flips_owner_to_support(use_case, tenant, owner_tenant_user, tenant_user_repository):
    with _patched_registerer(tenant):
...
async def test_execute__delegates_member_invitations(use_case, tenant, tenant_user_invitation_repository):
    """Integration check: the onboarder hands off to TenantMemberInviter,
    ...
    expect({inv.email for inv in result.invitations}).to(equal({"a@test.com", "b@test.com"}))
    tenant_user_invitation_repository.persist_many.assert_called_once()


async def test_execute__skip_email_propagates_to_inviter(use_case, tenant, command_bus):
