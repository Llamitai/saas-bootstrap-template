...
def use_case(tenant_id, tenant_user_invitation_repository):
    return PendingInvitationLister(
        tenant_id=tenant_id,
        tenant_user_invitation_repository=tenant_user_invitation_repository,
    )


async def test_execute__returns_pending_within_ttl(use_case, tenant_id, tenant_user_invitation_repository):
...
async def test_execute__filters_out_pending_but_past_ttl(use_case, tenant_id, tenant_user_invitation_repository):
...
async def test_execute__empty_when_repo_returns_empty(use_case, tenant_user_invitation_repository):
    tenant_user_invitation_repository.list_pending_by_tenant.return_value = []
