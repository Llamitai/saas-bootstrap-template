...
class RedisTokenStore(TokenStore):
    ...
    async def get_token_jti(self, sub: str, namespace: str) -> str | None:
        # redis-py no es genérico sobre decode_responses; el tipo real depende del cliente.
        return await self.redis_client.get(f"{namespace}_RT:{sub}")  # ty: ignore[invalid-return-type]
    ...
    async def clean(self, jti: str, sub: str, namespace: str) -> bool:
        ...
        return bool(await self.redis_client.delete(f"{namespace}_BL:{jti}"))
