...
They reuse `TenantMemberInviter`, `PendingInvitationLister`, and
`InvitationCanceller` so the business logic is the single source of truth.
"""
...
from src.tenants.application.use_cases.invitations.canceller import InvitationCanceller
from src.tenants.application.use_cases.invitations.inviter import MemberInvitationInput, TenantMemberInviter
from src.tenants.application.use_cases.invitations.pending_lister import PendingInvitationLister
...
async def create_tenant_invitations(
    ...
    result = await TenantMemberInviter(
        tenant=tenant,
...
async def list_tenant_invitations(
    ...
    tenant = required_tenant_for(current_tenant_user)
    invitations = await PendingInvitationLister(
...
async def cancel_tenant_invitation(
    ...
    tenant = required_tenant_for(current_tenant_user)
    invitation = await InvitationCanceller(
