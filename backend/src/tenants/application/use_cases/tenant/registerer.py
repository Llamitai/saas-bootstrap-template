...
from src.common.constants import TENANTS_LIMIT
...
class TenantRegisterer(UseCase):
    ...
    tenant_user_repository: TenantUserRepository | None = None
    ...
    async def execute(self) -> Tenant:
        ...
        assert tenant is not None  # persist re-reads the row it just flushed
    ...
    async def _check_creation_limit(self) -> None:
    ...
    async def _bootstrap_roles(self, tenant: Tenant) -> None:
