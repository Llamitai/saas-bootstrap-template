...
async def get_user_tenants(
    ...
    user_tenants: list[Tenant] = cast("list[Tenant]", result) if isinstance(result, list) else []

    return ApiJSONResponse(content=[tenant.model_dump(mode="json") for tenant in user_tenants])
