...
class TenantUserMixin:
    ...
    async def _get_tenant_user(self) -> TenantUser:
        tenant_user = cast(
            "TenantUser | None",
            await self.query_bus.ask(query=GetTenantUserByIdQuery(tenant_user_id=self.tenant_user_id)),
        )
...
class TenantRoleValidatorMixin:
    ...
    async def _assert_tenant_role_exists(self, tenant_role_id: UUID) -> None:
        tenant_role = await self.query_bus.ask(query=GetTenantRoleByIdQuery(tenant_role_id=tenant_role_id))
    ...
    async def _get_tenant_role(self, tenant_role_id: UUID) -> TenantRole | None:
        return cast(
            "TenantRole | None",
            await self.query_bus.ask(query=GetTenantRoleByIdQuery(tenant_role_id=tenant_role_id)),
        )
