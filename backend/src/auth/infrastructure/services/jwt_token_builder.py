...
class LegacyJWTTokenBuilder(LegacyTokenBuilder):
    async def create_access_token(self, user: User) -> JwtSession:
        access_expire = datetime.now(tz=UTC) + timedelta(minutes=settings.JWT_ACCESS_TOKEN_EXPIRE_MINUTES)
        access_payload = {
            **user.token_data,
            "exp": access_expire,
            "type": "access",
        ...
        refresh_expire = datetime.now(tz=UTC) + timedelta(minutes=settings.JWT_REFRESH_TOKEN_EXPIRE_MINUTES)
        refresh_payload = {
            "sub": str(user.uuid),
            "exp": refresh_expire,
            "type": "refresh",
