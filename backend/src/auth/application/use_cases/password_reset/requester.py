...
@dataclass
class PasswordResetRequester(UseCase):
    email: str
    user_repository: UserRepository
    token_service: TokenService
    command_bus: CommandBus
    reset_base_url: str = field(default_factory=lambda: settings.FRONTEND_HOST or "http://localhost:3000")
    ...
    async def execute(self) -> None:
        ...
        reset_url = f"{self.reset_base_url.rstrip('/')}/reset_password/{token}"
