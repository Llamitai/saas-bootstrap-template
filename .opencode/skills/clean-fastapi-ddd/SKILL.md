...
- Use cases are named `[Subject][Action]` with the verb as an agent noun (`-er`/`-or`) **last** — `ConnectionAccountCreator`, never `CreateConnectionAccount`. (Verb-first is for `Command`/`Query`.) See `references/use-cases.md`.
- Every use case is a `@dataclass` (never a hand-written `__init__`); add `__post_init__` **only when necessary** — a default that can't be a literal, or a field derived from others. See `references/use-cases.md`.
- One use case per file; the file name is the shortest action suffix the folder already disambiguates — `tenant_users/creator.py` for `TenantUserCreator`, never `tenant_user_creator.py` or a multi-use-case module. See `references/use-cases.md`.
