...
function PageContentHeader({
  ...
  return (
    <header
      ...
      className={cn(
        "sticky top-0 z-20 mb-6 flex shrink-0 items-center gap-4 bg-background/95 px-6 pt-6 supports-[backdrop-filter]:bg-background/80 supports-[backdrop-filter]:backdrop-blur-sm",
        className
      )}
    >
      {showBack && (
...
function PageContentBody({ children, className }: PageContentBodyProps) {
  const inset = usePageContentBottomInset();
  return (
    <div
      data-slot="page-content-body"
      style={inset > 0 ? { paddingBottom: inset } : undefined}
      className={cn(
        "flex min-h-0 flex-1 flex-col overflow-y-auto px-6 pb-6",
        className
      )}
    >
      {children}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "bg-background text-foreground shadow-whisper ring-1 ring-inset ring-border/60"
